...
static lv_point_t trigger_cursor_A_points[] = { {CHART_LEFT, CHART_TOP+400}, {CHART_X+CHART_LEFT, CHART_TOP+400}};
static int vertical_mode = 0;   //  垂直档位, 0-0.1V/div; 1-0.2V/div; 2-0.5V/div
static int horizonal_mode = 0;   //  水平档位, 0-1us/div; 以此类推
static double vertical_cursor_A_value_show = 0.50;  //  垂直光标A的当前值
static double horizenal_cursor_A_value_show = 0.00;  //  水平光标A的当前值
static double vertical_cursor_B_value_show = 0.50;  //  垂直光标B的当前值
static double horizenal_cursor_B_value_show = 0.00;  //  水平光标B的当前值
static double vertical_cursor_Diff_value_show = 0.00;  //  垂直光标A的差值
static double horizenal_cursor_Diff_value_show = 0.00;  //  水平光标A的差值
...
static void vertical_cursor_slider_event_handler(lv_obj_t * obj, lv_event_t event)
{
    if(event == LV_EVENT_VALUE_CHANGED) {
    	...
    	if(vertical_cursor_switch == 0){
       		vertical_cursor_A_value = CHART_BOTTOM - slide_value;
            ...
            vertical_cursor_A_value_show = (vertical_mode == 0)?(-0.500+0.002*slide_value):(vertical_mode == 1)?(-1.000+0.004*slide_value):(-2.50+0.01*slide_value);
            ...
            lv_table_set_cell_value(table, 5, 1, vertical_cursor_A_string);
       		...
       		vertical_cursor_B_value = CHART_BOTTOM - slide_value;
            ...
            vertical_cursor_B_value_show = (vertical_mode == 0)?(-0.500+0.002*slide_value):(vertical_mode == 1)?(-1.000+0.004*slide_value):(-2.50+0.01*slide_value);
            sprintf(vertical_cursor_A_string,"%.3lf",vertical_cursor_B_value_show);
...
static void button_channel1_event_handler(lv_obj_t * obj, lv_event_t event)
{
    if(event == LV_EVENT_CLICKED) {
    	channel_select = 0;
    	channel_show = "CH1";
        lv_table_set_cell_value(table, 0, 1, channel_show);
    }
}
...
void update_osc_task(lv_task_t * task)
{
  ...
  /*Do something with LittlevGL*/
	if(stop_sampling == 0){
		...
		for(i = 0; i < 100; i ++){
//			if((signed char)(ADC_RxBufferPtr[i]) - (signed char)(ADC_RxBufferPtr[i - 1]) > 80
//					|| (signed char)(ADC_RxBufferPtr[i - 1]) - (signed char)(ADC_RxBufferPtr[i]) > 80)
//				ADC_RxBufferPtr[i] = ADC_RxBufferPtr[i-1];
			points_array_channel1[i] = (lv_coord_t)(((signed char)(ADC_RxBufferPtr[i]) + 128) * 100 * SCALE_1V / 256);
			points_array_channel2[i] = (lv_coord_t)(((signed char)(ADC_RxBufferPtr[i] >> 8) + 128) * 100 * SCALE_1V / 256);
...
void osc_scope_ui(void){
    ...
    lv_slider_set_range(vertical_cursor_slider, 0, CHART_Y);
...
//    lv_label_set_style(label_trigger_cursor, LV_LABEL_STYLE_MAIN, &style_font_cursor_label);
    ...
    lv_table_set_cell_value(table, 5, 0, "V-A(mV)");
    lv_table_set_cell_value(table, 6, 0, "V-B(mV)");
    lv_table_set_cell_value(table, 7, 0, "H-A(us)");
    lv_table_set_cell_value(table, 8, 0, "H-B(us)");
    lv_table_set_cell_value(table, 9, 0, "VA-VB(mV)");
